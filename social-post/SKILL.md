...
**KPI 換一套**：鉤住率 ／ 略過率 ／ 平均觀看秒數 ／ 完播曲線，不是 FB 的非追蹤比例。**YT Shorts 的數據與訓練在自動剪輯專案**（開源版 ＝ video-autopilot-kit，channel_state 追蹤器為唯一真值；判讀走其 yt-algorithm-mastery 的續看率三級制）—— 已入庫三支 Shorts 的 D2 三平台對照：**同片 YT 1,512-1,735 vs FB 308-490 vs IG 189-219**，Meta 那套數字不可外推。這一層所有結論一律 🧪，不進 R 編號、不寫成鐵則。
