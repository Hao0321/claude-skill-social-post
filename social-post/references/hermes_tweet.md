...
用途：找 X 上正在討論的題材，補 `current_brief.md` 的題材提示。
...
發完要把回傳連結或 tweet id 寫進同一筆 `posts.jsonl` outcome。
...
- 頻率
- 停止時間
- 要寫回 structured outcome ledger 的欄位
- 是否只通知顯著變化
...
## 寫回 Structured Outcome

發文後透過 outcome bundle 在同一筆 post 記錄：
