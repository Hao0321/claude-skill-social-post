...
## v1.3.2 — 2026-07-23（📬 野生使用者回饋：README 升級指南 + Case 45）

- 🆕 **README 新增「升級（舊版 → 最新版）」章節**：來自真實使用者留言「舊版要升級要下什麼指令？」—— repo 從 v0.x 發到 v1.3.x 一直沒有升級指南。三步升級，明確保護使用者自己的 `style_profile.md` 與 `content_plan.md` 不被覆蓋。**留言區是免費的產品 roadmap。**
- 📚 **Case 45：skill 更新 meta 公告文**（1,913 瀏覽 ／ 23.7% ／ 儲存 12 > 分享 3）—— 同意圖連發第 4 篇的代價，本週第三次實證 R5 ／ R1 ／ R2 疊踩：內容品質沒崩，是排程紀律崩了。儲存密度 1.14% 與 mega 同級 ＝ 工具型受眾「收藏 > 轉發」的正字標記。
- 🔧 修正 F30 示範裡的星期錯誤（7/24 是星期五，原誤標星期四）。

**R1-R42／F1-F30／Cases 1-45 + F19 部署包。**

---

## v1.3.1 — 2026-07-22（🔧 五個矛盾清除 + 貼文索引回填至 15 列）

- 🔧 **F6b 月配額三方矛盾**（上限表「每月 1 次」vs 月配額表「2 篇」vs 條目「月內 ≤ 2 次」）→ 舊上限表標廢止，一律以公式條目為準。
- 🔧 **F18 ／ F26 的 7h ／ 6h 早期快照兩個月未回填** → 標「依 R6 不可當定論，實質等於無實證」。
- 🔧 **F1 平台標籤衝突**（條目 Threads ／ X 主場，導航卻預設 FB）→ 加 🧵✖️ 標籤，與 F7 同類錯誤。
- 🔧 **F30 掛進 Mode 表**（原為唯一沒歸屬任何 Mode 的公式），註明 KPI 看報名數。
- 🔧 **F19 排版鐵則反例 n=1 → n=2**（兩篇夥伴的多行版仍分別打出 53 萬與 42.2 萬），條目加 🧪 註腳。
- 📅 **貼文索引回填 Cases 1-7（7 → 15 列）**：揭出最硬的 R4 證據 —— **同一個 02:13 黃金窗，四篇結果差 500 倍**（80,050 ／ 152 ／ 1,578 ／ 263），時段不帶 viral，敘事意圖才帶。

**R1-R42／F1-F30／Cases 1-44 + F19 部署包。**

---

## v1.3.0 — 2026-07-21（🔬 全庫審計：刪掉一條沒有出處的規則）

四維度審計（規則／公式／案例提煉／跨檔漂移）＋ 對抗式過濾。**最重要的成果是刪除，不是新增** —— 提出的升級提議有 18 條被判定為「為改而改」直接砍掉，零新增 R 編號、零新增 F 編號。

### 🔴 R24 量化階梯全庫刪除（五處）

原本寫「1 篇孤立 = 50K ／ 2 篇 = 100K ／ 3 篇 = mega 150K+」，審計後確認**整條站不住**：

- **50K 與 100K 兩階全 repo 查無出處**。唯一的「孤立 F19」實測是 **16K**。
- **150K 是三篇的平均值被誤讀成「第 3 篇的天花板」**：來源序列實際是 26 萬 ／ 6.9 萬 ／ 12.7 萬，平均 15.2 萬。
- **來源序列本身反證累積說** —— 破紀錄的 26 萬正是**序列的第 1 篇**（孤立），之後遞減。
- 整個階梯是在兩個 n=1 數據點之間**內插**出來的。

刪除範圍：`rules.md` 條目、`SKILL.md` 速查、`case_studies.md` 正本階梯表、**`F19_DEPLOYMENT_KIT.md` 第九節**（這是實際發給夥伴照做的手冊，傷害最大）、`youtube.md`。保留無爭議的「純血可連發但每篇主題須各換」，整條降 🧪。R21 廢除理由同源誤讀，標「待重驗」。

### 📐 新增〈證據強度公約〉（不佔 R 編號）

全 repo 通用升級門檻：proven 需 **n ≥ 2 個彼此獨立、confound 可分離**；n=1 或 confound 糾纏一律 🧪 emerging 且不得寫成鐵則。附「獨立」的定義與兩條鐵律（**沒有出處的數字一律刪**、**沒有新增也是合格的結論**）。此前這個門檻只寄生在 R42 裡，導致三處誤引成「R41 標準」，一併更正。

### 🧪 兩條規則降級 + 三處杜撰刪除

- **R41 superlative** 降 emerging：唯一實證是 n=1 A/B，且兩篇差了平台社群、17 天、Reel 格式，表頭「唯一變數」不成立。
- **R38 第三方戰績** 降 emerging：n=1，且與 R36 共用同一組對照，同一證據被兩條規則各記一次功。
- 刪除 **F13**「5 個 tag 平均覆蓋 5-15K」「比 F4 擴散 5 倍」、**F11**「Meta AI 2025 新增 perceived authenticity score」—— 全 repo 查無出處。
- SKILL.md 速查表同步補 🧪（R38／R40／R41）。

### 🩺 evaluation.md 誤診修正（風險最高的兩行）

- 「追蹤者比例 < 15% = 鎖鐵粉圈」語意倒置 —— 門檻掛在**非追蹤者**比例。照原寫法會把 Day 1 mega（追蹤者比例 5.9%）診斷成鎖鐵粉。
- 互動率「> 15% = 優秀」補上 **只適用深化型**的 caveat（擴散型 mega 同口徑也只有約 1.3%），分母統一用「瀏覽者」。

### 🔢 數字對齊

7/20 過期 12h 快照 → 33h 值（83.8% ／ 186 ／ 303）；dwell proxy 打字錯 1.75 → 1.84；R39 分享排名補進遺漏的 87／37／35；R18 儲存階梯補 303 與 139；診斷路由補載 R39／R40。

**R1-R42／F1-F30／Cases 1-44 + F19 部署包。**

---

## v1.2.0 — 2026-07-21（🧭 公式分類導航 + F30 活動宣傳型 + Case 44）

大改版：29 個公式散在四個 Part 裡難以檢索，這版加一層**意圖分類導航**，並補上長期缺席的活動文公式。

- 🧭 **formulas.md 開頭新增公式分類導航**：七區（萬用／推廣／破圈／心得／社群／活動／句式庫），F1-F30 逐一歸類，開頭附「先問自己一句話」對照表。**F 編號完全不動**（rules.md 有大量交叉引用），只加一層檢索。
  - **平台改成標籤不當分類**：🧵 = Threads only（F7／F19）、✖️ = X／Threads 主場（F5），其餘預設 FB。避免「今天要在 Threads 推剛 ship 的東西」不知道翻哪一區。
  - **推廣款內部二分**：2-A ship 公告型（F6／F21／F29）vs 2-B 開源送出型（F11／F15／F26／F27），並寫死「推廣 vs 破圈」判準（有第三方可查數字 → 走 F28）。
  - Mode A／B／C 是 voice 軸，跟本分類正交，voice 鐵則不受影響。舊的〈目標 → 公式對應〉表標為已被取代。
- 🆕 **F30 活動宣傳型（🧪 emerging，n=1 反面實證）**：處理活動文的 **地理 × 時間 × 題材三重 niche 天花板** —— 天花板消不掉只能繞過，做法是**讓去不了的人也有東西可拿**（記錄／回放），這也是活動文唯一能拿到儲存的槓桿。**KPI 改看報名數，不看觸及。** 明確標註只有反面實證、上限未知，不記硬數字。
- 📚 **Case 44：活動文 15.4% 非追蹤**（分享 1 ／ 儲存 0 ／ 長期曲線低於自身基準線）—— 頭號死因是第一段就放時間地點，把 99% 路人篩掉；全篇唯一普世的「社群長出自己的支線」被埋在第四段。附照 F30 重寫的示範。

**R1-R42／F1-F30／Cases 1-44 + F19 部署包。**

---

## v1.1.6 — 2026-07-21（🔬 Case 43：深化型的正面樣本 + 相鄰日 artifact 對照）

- 🆕 **Case 43：純金句立場文**（4,537 瀏覽 ／ 非追蹤 31.7% ／ 分享 10 ／ 儲存 4，但**互動率 9.54% 為同批最高**、留言全是讚美型）。發在前一篇 mega 之後約 24.5 小時、同時段、同主題，是**幾乎隔離 artifact 變數的相鄰日對照**：有 artifact 的分享 186 ／ 儲存 303 ／ 非追蹤 83.8%，純金句版是 10 ／ 4 ／ 31.7%（分享差 18.6 倍、儲存差 75 倍）。
- 🆕 **R6 補上「深化型」的正面樣本**：低非追蹤比例不等於失敗 —— 擴散型看非追蹤比例、深化型看互動率與留言質，**兩把尺不能互換**。先前檔案裡只有反面樣本。
- 🆕 **R2 擴充（🧪 emerging）：冷卻要看「爆款有沒有停止成長」，不是死抓 24 小時**。前一篇仍在爬坡時發同敘事意圖的下一篇，可能被吸走演算法配額。⚠️ 三因糾纏（沒 artifact ／ 配額被吸 ／ 同題材疲勞）無法分離，故標 emerging。

**R1-R42／F1-F29／Cases 1-43 + F19 部署包。**

---

## v1.1.5 — 2026-07-21（📈 Case 42-C 33h 更新：非追蹤比例往上爬 = 第二波還在放量）

- 📈 **Case 42-C（value-prop 長文）33 小時數據**：12h 的 27,526 ／ 81% ／ 分享 127 ／ 儲存 232 → 33h 的 **39,859 ／ 83.8% ／ 分享 186 ／ 儲存 303**，收益 $1.35 → $1.93。**分享 186 逼近史上最高 195，儲存 303 為史上第二。到 33h 仍未 plateau。**
- 🆕 **R6 新增判讀訊號：非追蹤比例「往上爬」= 第二波廣推正在放量，別急著收單**。一般長尾是追蹤者先看完、陌生人零星補 → 比例往下掉；比例往上爬代表演算法第二波在 12h 之後才真正放量，這時候記的數字一定會被自己推翻。
- 🔧 **時間更正**：C 篇實際發文時間為 07-20 **上午 11:14**（先前由「12 小時前」反推成約 12:10）。索引表與 Case 42 同步。
- 🔧 R40 dwell proxy 數字同步（7/20 長文 1.75 → 1.84）。

**R1-R42／F1-F29／Cases 1-42 + F19 部署包。**

---

## v1.1.4 — 2026-07-21（🔬 面板讀法修正：私訊分享隱形 + dwell proxy + 互動率兩套標準）

對 Case 42-A（分享只有 1 卻 broke 51.1%）做深度歸因後，修正三個會導致誤判的面板讀法。

- 🆕 **R39 補充：面板的「分享」只計公開分享**。私訊分享（Messenger）才是最強信號（R15）卻**不進面板** —— 會得罪人的題材（反講師 / 幸災樂禍）公開轉發等於當眾站隊、掛名成本高，擴散多半走私訊。**看到分享個位數不等於引擎沒點火。** 同時標出這一 class 的結構性天花板落在 50-58%（Case 32 分享 2 → 57.9%、Case 34 分享 6 → 48.9%、Case 42-A 分享 1 → 51.1%），要上 83%／90% 必須點著公開分享與儲存兩個放大器。
- 🆕 **R40 新增面板可算的 dwell proxy**：`瀏覽次數 ÷ 瀏覽者` = 平均每人看幾次。實證帶 1.7-2.1，圖卡型（2.06）比長文型（1.75）更會拉 revisit。標明是 proxy 不是官方 dwell。
- 🆕 **R6 警告：互動率 > 15% 只適用深化型**，別拿去砍擴散型。同口徑下 Day 1 mega 也只有約 1.3%。算互動率分母要統一用「瀏覽者」，混用瀏覽次數會得出假的高低。
- 🆕 **R36 補充：「可驗證」不一定要有成品**。平台事實 / 人人可查的改動（「打開你自己的洞察就知道」）＝ 驗證成本近乎零且不需要相信作者，是 demo > claim 的無成品變體。

**R1-R42／F1-F29／Cases 1-42 + F19 部署包。**

---

## v1.1.3 — 2026-07-21（📅 貼文索引 + Case 42 三篇對照組）

- 🆕 **case_studies 開頭新增「貼文索引」表**：每篇記 **星期 ／ 時間 ／ 平台 ／ 格式 ／ 型 ／ 關鍵字 ／ 瀏覽 ／ 非追蹤% ／ 分享 ／ 儲存**，生成前可先查「同星期幾 + 同時段 + 同格式」過去打出什麼成績。
- 🏆 **Case 42：同一週三篇 FB 對照組（R37 教科書級實證）**：圖卡 news（51.1% ／ 分享 1 ／ 儲存 4）vs 純 CTA 公告（**40.9%，連鐵粉圈都沒破** ／ 分享 3）vs value-prop 長文（**81% ／ 分享 127 ／ 儲存 232**）。唯一單調變動的變數是「有沒有講清楚能幫你做什麼 + 有沒有東西可拿走」。
- 🆕 **R37 補充：純 CTA 公告 = 最差的一種貼文**（只喊「我要辦 X 了，連結在留言區」而不講價值 → 路人沒理由點、鐵粉沒理由轉）。
- 🆕 **R39 補充：話題驅動是第二條 broke 路徑**（借當週熱點，分享只有 1 仍打到 51.1% 非追蹤；但儲存只有 4 —— 話題借得到觸及，借不到儲存與分享）。
- 🔧 **R4「內容 > 時段」再證**：星期一中午發的長文打到 81% 非追蹤，三篇全在白天發、全不在黃金窗。時段是加分項不是門票。
- 🔧 **Case 41 日期更正**：原記 07-16 有誤，依實際星期六更正為 **07-18**。

**R1-R42／F1-F29／Cases 1-42 + F19 部署包。**

---

## v1.1.2 — 2026-07-17（📊 數據版：Case 39 plateau + 第五個夥伴複製 + 借/自產 artifact 分工）

- 📈 **Case 39 三週 plateau 更新**：4 天 44 萬 → **49.5 萬瀏覽 / 轉發 1,278 / 貼文帶來追蹤 4,468**。F19 長尾比 FB 長，早判會低估（R6 再證）。全庫紀錄引用同步（轉發率 0.258%、前高倍數 16.8x）。
- 🏆 **Case 41：第五個獨立夥伴複製，破生態轉發紀錄**：中型基數首例（約 5,400 粉），42.2 萬瀏覽 / **轉發 1,689（0.40%，生態雙紀錄）** / 分享 6,633，星期六正午發。公式在夥伴手上打出比原作者更高的深度指標 = 可攜性最硬證明。
- 🆕 **R19① artifact 分工 nuance**：artifact 不必自產 —— 第三方權威免費資源（權威 + 免費 + 零摩擦）一樣拉爆轉發；**但借的 artifact 拉觸及、自產的 artifact 拉追蹤**（漲粉轉化差約 7 倍，🧪 emerging）。部署包第八節同步。

**R1-R42／F1-F29／Cases 1-41 + F19 部署包。**

---

## v1.1.1 — 2026-07-16（🎨 全形標點 + 自然語言全庫打磨）

整庫品質打磨：指引與範例語句順成自然口語，標點全面統一全形。

- 🆕 **R34 新增「全形鐵則」小節**：中文語境內標點一律全形（，、。！？：；（）「」），半形只留給程式碼、URL、英文句子、數字千分位與小數點（42,103／17.4 這種絕不能動）。SKILL.md R34 速查列同步標註。
- 🎨 **全庫語句自然化**：生硬與翻譯腔語句順成台灣口語，規則語意、鐵則、實證數據一字未變。
- 📜 **歷史案例引文保留原貌**：case_studies 引用的真實貼文原文一字未動（含當時的標點），忠實保存戰績紀錄。
- ✅ **機械驗證零漂移**：每個檔案跑數字序列 + markdown 結構比對，戰績數據與表格結構全數保持一致。

**R1-R42／F1-F29／Cases 1-40 + F19 部署包。**

---

## v1.1.0 — 2026-07-04（🚀 F19 可攜性 proven + F19 部署包開源）

F19 立場宣言公式不只 Hao 本人有效 —— 多個素人夥伴帳號複製成功，升級為 proven 可攜公式，並開源部署手冊。

### 🆕 F19_DEPLOYMENT_KIT.md（Threads 破圈成長操作手冊）

可下放給夥伴照做的完整 playbook：五元素骨架 + 填空模板、排版鐵則、keyword 清單、反 AI 腔 voice、🔑 stance 綁 artifact 拉爆轉發、🔑 連發累積 momentum（R24）、Thread 三指標分軸讀法、giveaway 反摩擦、常見錯誤、誠實射程、兩個 worked example。自由 fork。

### 🆕 F19 可攜性（proven）

- **跨帳號多案複製驗證**：≥3 個彼此獨立的素人夥伴帳號，從 **100-600 粉基數** → 單篇 **+4,000~8,600 粉 / 17.4 萬~53 萬瀏覽**。baseline 全部近乎零 → 觸及非帳號自身可解 → **格式驅動 proven**。
- **⚠️ 誠實射程**：目前案例共用同題材（反付費課程）+ 同話題波 → proven = 「溫熱話題波內可攜」；冷話題 / 離波未證，別 over-claim。

### 🆕 R19 擴充④ Thread 三指標分軸讀法

轉發（identity 表態，最深）≠ 分享（擴散，中）≠ 瀏覽（最淺）。「高瀏覽 + 低轉發率」= 綁 artifact 不足。**高瀏覽不等於高轉發。**

### 📚 Case 40

夥伴帳號純血 F19「反付費課程」：500-600 粉 → 53 萬瀏覽 / 9,220 粉（vs Hao Case 39 綁 artifact 版轉發 1,197）。

**R1-R42 / F1-F29 / Cases 1-40 + F19 部署包。**

---

## v1.0.4 — 2026-07-02（🔧 修正）

移除 v1.0.3 殘留的 3 處 YT 導流交叉引用（不屬於通用 skill 的私域營運細節）。
...
3. **Day 1 實證連結點擊率 = 0.016%**（13/80K）= 連結沒實際 ROI
...
**反差數字 pattern**（從 F14 偷學）：「Day 1 448 讚帶 +1,319 入群 / Day 5 9 讚帶 0 入群」
...
- **變體 A：promo 型（self-evident 鉤子）** — Day 1 mega-viral 範本（80,050 觀眾、448 讚、87 分享、+1,319 Line 社群）
...
- 外部連結點擊率在 mega-viral 中極低（Day 1 只 13/80,050），社群成長主路徑是留言 → 作者私訊拉人
...
專攻「突破鐵粉圈觸及非追蹤者」。Day 1 的 mega-viral（80K 觸及、94.1% 非追蹤者）是例外，多數創作者連續 3 篇卡 > 85% 追蹤者時必用這些公式強制擴散：
...
- F6b 驗證數據升級為 **72h plateau 最終版**：瀏覽 139,761 / 80,050 獨立觀眾 / 448 讚 / 500 留言 / **87 分享** / +167 粉 / **+1,150 Line 社群（~800→1,952）** / 最近 10 篇排名 1/10 / 72h 後仍在漲 = evergreen 長尾
...
4. 外部連結點擊率極低（Day 1 的 80K 人只 13 人點 GitHub）→ CTA 要放 FB 站內
...
### Changed
- F6b 驗證數據從「300+/400+/40+」升級到**實戰完整數據**：瀏覽者 80,050、互動率 1.46%、讚 448、留言 500、分享 87、追蹤者 +167、Line 社群 +700。
- `references/formulas.md` F6b 段新增反直覺觀察：連結點擊率極低（80K 讀者中只 13 人點外部連結）→ 社群成長主路徑是**留言 +1 → 作者私訊拉人**。

### Lessons learned（可複製給其他 4K 粉帳號的心得）

1. **爆款觀眾 94.1% 是非追蹤者** — F6b 觸發演算法廣推，不靠既有粉絲。
