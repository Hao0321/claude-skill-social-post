...
- **「IG 觸及跳升 ＝ 技能有在長」補上保留**：Case 48 裡本來就標了「題材同時換了、分不開」，但 SKILL.md 只留了前半句。已補 —— **別把它當技能練起來的證據。**
- **Case 46 的「IG 推不動你的片」補上自我推翻**：兩週後 Case 48 的稀有生態片在 IG 拿 1,837 人。「先在有受眾的平台養」修正為 **「先問這個題材的受眾在哪個平台」**。
- **短影音端的門檻缺口明寫**：R18（儲存 ≥ 22）與 R39（分享 ≥ 20）是從四萬瀏覽量級的 FB 純文字校出來的，**套到 200-1,800 人的 Reel 會把每一支都判死**。短影音端目前沒有有效門檻 —— **寧可空著也不要借數字填。**
- **「IG 前台讚數 ＝ IG ＋ FB 合計」升到 proven 並搬進 evaluation.md**：兩組精確恆等式（87 ＝ 32 ＋ 55、59 ＝ 0 ＋ 59），而且 IG 端留言是 0 前台卻顯示 59 —— 不可能是標籤問題。它原本只活在標 🧪 的 Case 48 裡，會被連坐降級。
- **「1.2-1.3 重播率」明寫不是門檻**（FB 對照 n=3、IG n=6），**玩具開箱修正後仍 2.36 標為原因未定**（不用「圖片計次不同」這種 n=1 的事後解釋填）。
- **兩批 IG 表補時窗與欄位原則**：Case 46 是「觀看 ／ 觸及帳號」、Case 48 是「瀏覽次數 ／ 瀏覽人數」—— **欄位名逐字照抄面板，不做正規化，欄位名就是時間戳**。v1.6.0 刪掉的那條循環論證，病因就是欄位名被改寫。

---

## v1.6.0 — 2026-08-10（📱 IG 第二批 ＋ 刪掉一條循環論證的規則）

同一週七支 IG 貼文（六支 Reel ＋ 一支圖片）。但**這版最重要的不是新增，是刪掉一條我自己寫錯的現役規則**。

### 🔴 刪除：「同批連發有順位懲罰，好片放前面」是循環論證

R17 與 Case 46 曾寫「三支 Reel 鉤住率幾乎一樣，排第 2 的拿 1,040、排第 10 的只有 114，所以同批發文排序越後面吃越少配額 —— 好片要放前面」。

**那個 N/10 不是發文順序。** FB 面板的原意是「這篇在**最近 10 篇中排名第 N**」＝ **依表現排出來的名次**（定義本來就寫在 evaluation.md，是轉錄到 Case 46 時被改寫成「同批排名」，再被讀成「同批 10 支」）。1,040 高所以排第 2、114 低所以排第 10 —— **用名次解釋瀏覽，等於把同一個數字講兩次**。

- 「好片放前面」這條排程建議**作廢**。
- 倖存的觀測改寫成站得住的版本：**三支鉤住率幾乎相同（29.6% ／ 28.8% ／ 29.8%），瀏覽卻差 9 倍 —— 鉤住率一樣不保證觸及一樣**。9 倍差距的原因**未定**，不硬給。
- evaluation.md 補上 N/10 的**唯一權威定義**，並明寫「名次由表現導出，不可反過來用名次解釋表現」。

### 🔧 同時清掉一批過期資產

- **續看率三級制（≥70% ／ ~60% ／ ≤40%）已於 2026-08-02 被上游校準作廢**（反例：續看 31.1% 照樣拿 1,735 views，且三支內續看率與 views 反向），但 v1.5.0 把它當現役鐵則寫進 R17 ／ SKILL.md ／ CHANGELOG。全部改為「**續看率＝診斷不是預測，生死看 views 量級本身**」。
- **新增「平台數據只認該平台自己的後台」**：IG 面板底部那個 Facebook 區塊只算 IG→FB 轉發，實測跟 FB 原生後台差 **40-70 倍**（7 ／ 11 ／ 4 vs 490 ／ 308 ／ 392）。看那欄會得出「FB 已死」的完全錯誤結論。
- 三平台配比標籤修正：**「瀏覽配比」不是「觸及配比」**（算式來源是 views 不是 reach），YT : FB : IG ≈ 8 : 2 : 1，結論不變 —— 要省力砍 IG 別砍 FB。
- 補進上游校準的其他結論：FB Reels **3 秒以上僅 25-27%、平均觀看 4-6 秒**（比 YT 更沒耐心）；**Shorts ≒ 不變現也不轉訂**（三平台總收益約 US$0.06、訂閱轉換 0.06%）。

### 🆕 Case 48

- **短影音這個技能有在長**：IG 觸及從首批的 177-212 跳到 **456-1,837**；分享與儲存從**全部是 0** 變成 20 ／ 10 ／ 6 和 7 ／ 5 ／ 2。⚠️ 題材同時換了，成長歸因分不開，標 🧪。
- **🧪 IG 端吃地標與奇景，不吃私人生活切片**：旅遊景點片觸及 223-1,837 ／ 非粉絲 88-98%；生活興趣片只有 **39 ／ 102 人**，開箱那支 **96.7% 鎖粉絲**、形同沒進推薦流 —— **差 20-47 倍**。
- **🏆 七支裡只有 AI 短劇帶來追蹤（+4）**，也是唯一儲存上 7 的。差別在 caption 是完整的工法揭露長文，不是一句地標 —— **等於 F6b meta-ship 換成影片載體**。旅遊線在練手感，AI 線才會轉成追蹤與信任。
- **🧪 景點類短影音出現「分享 ＞ 按讚」**（海濱 20 ／ 10、步道 6 ／ 5，n=2 首次）。景點內容的主要動作本來就是傳給朋友，別拿按讚數判死活。
- ⚠️ **這個 Case 我自己踩了坑並留在檔案裡當警示**：原本用 IG 面板那個 FB 欄推出「題材各有主場」，但那欄不是 FB 真值 —— 已刪掉該推論，只留用 IG 自己欄位撐得住的部分。

### ⚠️ IG 欄位口徑（🧪，刻意只寫觀測層）

跨發到 FB 的貼文，**IG 前台顯示的讚數與留言數是 IG ＋ FB 合計** —— 前台「87 讚 ／ 59 留言」拆開是 IG 32 ＋ FB 55、IG 0 ＋ FB 59，兩項逐位吻合。insights 的「瀏覽次數」也含一個不進「瀏覽人數」的 Facebook 分項（減掉後六支 Reel 的次數 ÷ 人數從 1.22-6.11 收斂到 1.15-1.36，正好落進 FB 原生後台的 1.18-1.34 同一帶）。

**但不寫成「那一格 ＝ FB 真實瀏覽」** —— 同帳號一週前的紀錄與此矛盾，兩者必有一邊要改寫，目前無資料判定。**不管真相如何，操作建議都成立：跨平台比較一律用「瀏覽人數 ／ 觸及」。**

**R1-R42／F1-F30 編號不變。刪掉的比新增的重要。**

---

## v1.5.1 — 2026-08-04（🔧 Case 47 數據勘誤：卡片概數 ≠ 實際星數）

- Case 47 樓中樓連結導去的 repo 是**自動剪輯 skill（video-autopilot-kit）**，先前記錄未寫明。
- 連結卡片顯示的「2k stars ／ 269 forks」是 **GitHub 卡片概數**，先前被當精確值寫進 Case 47 ／ R25 ／ SKILL.md 時代二。2026-08-04 GitHub API 查核實際為 **1,693 stars ／ 278 forks**，四處已更正（卡片顯示內容保留為歷史記錄，附查核註）。
- 對照結論不變：Thread 樓中樓千級星數轉換 vs FB 正文四萬瀏覽 2 個連結點擊。

---

## v1.5.0 — 2026-08-04（🔍 F6b 全普查勘誤 + YT 訓練管道接通）

作者抓到兩個記錄斷層：F6b 的使用次數被記少了，YT Shorts 的數據其實一直在另一個專案裡累積。這版是一次「對帳」release —— 修錯的、補漏的、把管道接起來的，沒有新規則。

### 🔍 F6b 全普查（n=6 → n=8，兩顆 mega → 三顆）

- **勘誤**：R4 星期二表舊版漏掉 05-12 與 06-09 兩場，且 04-28 記 2,049、04-29 記 427 —— 那是早期快照的「瀏覽次數」，跟其他列的 plateau 觀眾數混了軸也混了時窗（04-28 plateau 實為 18,603 觀眾 ／ 92.6%、04-29 為 263）。全表已逐筆對回戰績記錄、統一 plateau 觀眾軸重建。
- **新結論**：**三顆 mega（4/21 的 80,050 ／ 5/5 的 44,110 ／ 6/9 的 42,103）全部落在星期二凌晨**；星期二凌晨 5 場 → 3 mega ＋ 1 mid-viral ＋ 1 fail（fail 那場另有已記錄死因），非星期二 3 場全部 < 1,000。confound 仍未分離，維持 🧪。
- 貼文索引補上 05-05 ／ 05-12 ／ 06-07 ／ 06-09 四列（只補有記錄時間的，不造時間）。
- F6b 條目標題從「雙實證」改「三顆 mega 實證」，SKILL.md 時代一同步。

### 🎬 時代三勘誤：「YT Shorts 零資料」作廢

- YT Shorts 數據一直在作者的自動剪輯專案（開源版 ＝ [video-autopilot-kit](https://github.com/Hao0321/video-autopilot-kit)）累積 —— 三支 Shorts 已有三平台 D2 對照。
- **R17 新增同片三平台對照（n=3）**：同一支片，YT Shorts 1,512-1,735 觀看（非訂閱 96.5-99.9%）vs FB Reels 308-490 vs IG 189-219 —— 隨意發線短影音的觸及主場在 YT Shorts feed，且完全不吃粉絲基礎。
- **D2 判讀窗口警告**：D2 流量 97%+ 來自 Shorts 推薦、搜尋 <1.2%，搜尋是 D28 之後的長尾 —— 別在 D2 判「搜尋型」死刑。
- **管道接通**：短影音數據唯一真值在自動剪輯專案的 channel_state 追蹤器（D2 ／ D7 ／ D28 快照制）；本 skill 只引用結論、不複製數據。之後 YT 演算法訓練的結論會持續融合進 R17 與時代三。
  - ⚠️ **v1.6.0 勘誤**：本條原本寫「判讀走其續看率三級制（≥70% ／ ~60% ／ ≤40%）」—— **那個三級制在 2026-08-02 就已被上游校準作廢**（反例：續看 31.1% 照樣拿 1,735 views），這裡同步漏了。正確講法是「**續看率＝診斷不是預測，生死看 views 量級本身**」。

**R1-R42／F1-F30 編號不變。刪錯的比新增的多，這是特色不是 bug。**

---

## v1.4.0 — 2026-08-04（🕰️ 三個引擎時代 + 射程標：終結跨平台誤用）

規則庫長期只有「強度」一個軸，沒有「平台」與「時代」—— 所以 FB 文字規則、Thread 專屬規則、還在養成期的短影音規則長得一模一樣，照著查會跨平台誤用。這版補上第二軸。

### 🕰️ SKILL.md 新增〈三個引擎時代〉

- **時代一 F6b（FB 破圈引擎，proven，可重複但要換燃料）**：兩顆 mega 都在**星期二 02:13**（4/21 的 80,050 與 5/5 的 73,622）。**5/5 是月內第 5 個 F6b 仍打同一級 —— 會膩的是敘事意圖，不是公式。** 但意圖有大小之分（4/28 同時段換小意圖只有 2,049），連發同受 R5 與 R42 管轄。
- **時代二 F19（Thread 可攜成長引擎，proven，現役導流主力）**：51 萬 plateau ／ 4,491 追蹤 ／ 樓中樓連結換到 2k stars，且五個獨立夥伴帳號複製成功。
- **時代三 短影音（🧪 訓練中）**：與上面兩層**不同層** —— 不是公式，是還沒練起來的技能。KPI 換成鉤住率 ／ 略過率 ／ 完播曲線。**YT Shorts 至今零資料**，不外推。
- 附「今天該用哪個」對照表。

### 🎯 rules.md 新增〈射程標〉（不佔 R 編號）

每條規則另掛三欄：**平台 ／ 時代 ／ 成熟度**。理由很實際 —— 「4 段 4 句」在 FB 是 proven、搬 Thread 必死；「keyword 留言 CTA」在 FB 最強、在 Thread 無效。

### 🏷️ 六處補上平台標，防止跨平台誤用

- **Viral 4 條件**標「FB 文字 Mode B 專用」，並附 Thread 對應式（1 段不換行 ／ 綁 artifact ／ keyword ≥ 2）。
- **R35 keyword CTA** 標「FB 專用」，**R20 漸進改進**標「Threads ＋ 含預估階 🧪」。
- **R6 四指標**與 evaluation.md 標明「FB 面板口徑」，並列出 Thread ／ 短影音各自的尺 —— **三套尺不可互套**。
- **R17** 改標「短影音養成期 ／ 訓練中」，招牌的 +50% 同日加成標為**未驗證外部宣稱**（R33 排定的量測從未回填）。
- **F6b 複製度**改寫：從「可無限期持續、每 4 天就能再爆」改成「殼可重用、meta 鉤子一次性、燃料是新意圖」，並把「連發沒問題」改成「連發有天花板」。

**R1-R42／F1-F30／Cases 1-47 + F19 部署包。零新增編號。**
